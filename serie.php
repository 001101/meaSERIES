...
$nb_episode = 0;
if ( count($seriesarray[$_GET['id']]['episodes']) > 0 ) {
	echo '<hr>';
	foreach ( $seriesarray[$_GET['id']]['episodes'] as $key => $saison ) {
		...
		foreach ( $seriesarray[$_GET['id']]['episodes'][$key] as $key2 => $episode ) {
			...
			echo '<form action="./update_episode.php" method="post"><b>Episode</b> <input type="hidden" name="serie" value="'.$_GET['id'].'"> <input type="hidden" name="saison" value="'.$key.'"> <input type="text" name="episode" value="'.$key2.'" size="2"> <input type="text" name="url_file" value="'.$url_file.'" size="50">&nbsp;&nbsp;';
